...
contract test{
    ...
    function getMyInfoArrayAlto() external view returns (Person memory) {
        
        Person memory persona;
        for (uint i = 0; i < myInfoArray.length; i++) 
        {
           if (persona.id<myInfoArray[i].id){
                persona=myInfoArray[i];
           }
        }
        return persona;
    }
    ...
    function sumaConstante() external pure returns(uint256){
        uint256 a=1;
       // return ( 2.5 + 2.5 + a );
  return ( 2.2 + 1.3+ 2.5 ); // Por que esta mal?
    }
...
// vimos hasta aca
    ...
    function valueSent() public payable returns(uint256){
        return (msg.value);
    }
